...
/**
 * Class ME_Options: equivalent to an option in wp_engine, we use this to group many options of our themes into 1
 * Create an option group in wp_options using the name provided when construct the object, e.g.:
 * 		$me_{theme}_general = new ME_Options("me_options");
 * Get & Set new option in this group using assignments & save() function, e.g.:
 * 		$me_{theme}_general->logo = {url};
 * 		$me_{theme}_general->save();
 * 		echo $me_{theme}_general->logo;
 ...
 * This class keeps the old ways of retrieving options, so you can also use $me_{theme}_general->get_option() & update_options(), add_options()
 * @author AnhCV
 * @version 1.0
 * @copyright enginethemes.com team
 * @license enginethemes.com team
 */
if (!class_exists('ME_Options')) {
    class ME_Options
    {
