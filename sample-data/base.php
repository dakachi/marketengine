...
class Base
{
    ...
    {
        if (!is_bool($strict)) {
            throw new Exception('randomNumber() generates numbers of fixed width. To generate numbers between two boundaries, use numberBetween() instead.');
        }
        if (null === $nbDigits) {
        ...
        if ($max > mt_getrandmax()) {
            throw new Exception('randomNumber() can only generate numbers up to mt_getrandmax()');
        }
        if ($strict) {
    ...
    public static function shuffle($arg = '')
    {
        ...
        throw new Exception('shuffle() only supports strings or arrays');
    }
