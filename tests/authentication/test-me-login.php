...
class Tests_ME_Login extends WP_UnitTestCase {
    ...
    public function test_login_wrong_pass() {
        ...
        $this->assertEquals(new WP_Error('incorrect_password', '<strong>ERROR</strong>: The password you entered for the username <strong>dakachi</strong> is incorrect. <a href="' . me_lostpassword_url('reset-pass') . '">Lost your password?</a>'), $user);
    }
    // invalid_username
    public function test_login_wrong_user_name() {
        $u1   = self::factory()->user->create(array('user_login' => 'dakachi', 'user_pass' => '123', 'user_email' => 'dakachi@gmail.com'));
        $user = ME_Authentication::login(array('user_login' => 'dakachi2', 'user_password' => '123'));
        $this->assertEquals(new WP_Error('invalid_username', 'Invalid username.'), $user);
    }
    /**
     * test login success
